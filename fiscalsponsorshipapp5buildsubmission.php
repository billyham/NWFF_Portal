...
if ($submitted === "1"){
    ...
    if (!file_exists("files/fiscal/$key_id.txt")){
    
        $fh = fopen("files/fiscal/$key_id.txt", 'w') or die("Failed to create file");
        $text = <<<_END
...
_END;
    ...
    $to = "$email";         
    $subject = 'Fiscal Sponsorship application confirmation for '."$first_name".' '."$last_name";

    $message = file_get_contents("files/fiscal/$key_id.txt");
